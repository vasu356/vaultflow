...
/**
 * Metadata service: manages object metadata, tags, version listing,
 * search by key prefix, and lifecycle rule enforcement.
 *
 * Uses JdbcTemplate for all queries — metadata queries are read-heavy,
 * analytically complex, and benefit from direct SQL rather than ORM overhead.
 */
@Service
...
public class MetadataService {
  ...
  public Map<String, Object> getObjectMetadata(UUID bucketId, String objectKey,
    ...
    var rows = jdbc.queryForList("""
        ...
        LIMIT 1
        """, bucketId.toString(), objectKey, principal.orgId());
  ...
  @Transactional
  public void updateMetadata(UUID bucketId, String objectKey,
    ...
    if (metadata != null) {
      jdbc.update("UPDATE object_versions SET metadata = ?::jsonb WHERE id = ?::uuid",
          toJson(metadata), versionId);
    }
    if (tags != null) {
      jdbc.update("UPDATE object_versions SET tags = ?::jsonb WHERE id = ?::uuid",
          toJson(tags), versionId);
    }
  ...
  public PageResponse<Map<String, Object>> listObjects(
      UUID bucketId, String prefix, String continuationToken,
      int maxKeys, VaultFlowUserPrincipal principal) {
    ...
    long total = Optional.ofNullable(jdbc.queryForObject("""
        ...
        """, Long.class, bucketId.toString(), principal.orgId(), likePrefix)).orElse(0L);
    ...
    var rows = jdbc.queryForList("""
        ...
        LIMIT ?
        """, bucketId.toString(), principal.orgId(), likePrefix, maxKeys);

    return PageResponse.of(rows, 0, maxKeys, total);
  }

  /**
   * Full-text search on object keys using PostgreSQL pg_trgm trigram index.
   * Production extension: add tsvector column on metadata JSONB for content search.
   */
  @Transactional(readOnly = true)
  ...
  @Transactional(readOnly = true)
  public List<Map<String, Object>> listVersions(UUID bucketId, String objectKey,
      VaultFlowUserPrincipal principal) {
  ...
  /**
   * Apply expiration lifecycle rules. For each bucket with lifecycle rules,
   * find objects older than the configured age threshold and soft-delete them.
   *
   * Called by the LifecycleScheduler every hour.
   */
  @Transactional
  ...
  private String getLatestVersionId(UUID bucketId, String objectKey, String orgId) {
    return Optional.ofNullable(jdbc.queryForObject("""
        ...
        """, String.class, bucketId.toString(), objectKey, orgId))
