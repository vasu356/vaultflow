...
/**
 * Storage read adapter for the download service. Mirrors LocalFileSystemStorage but is scoped to
 * read operations only. In production this would delegate to S3 GetObject with range support.
 */
@Component
public class StorageReadPort {
  ...
  public InputStream retrieveRange(String storageKey, long offset, long length) {
    ...
    try {
      ...
      raf.seek(offset);
      long readLength =
          length < 0 ? raf.length() - offset : Math.min(length, raf.length() - offset);
  ...
  public long getSize(String storageKey) {
    try {
      ...
      throw new StorageException("Size check failed: " + storageKey, e);
    }
  }

  private Path resolve(String key) {
    if (key.contains("..") || key.startsWith("/"))
      throw new StorageException("Invalid key: " + key);
