 ...
 *
 * @ConditionalOnWebApplication ensures this bean is only created in servlet-based
 * web apps (not in processing-service or notification-service which are pure Kafka consumers
 * with no HTTP controllers, but happen to have spring-web on classpath for actuator).
 */
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiErrorResponse> handleVaultFlowException(
    ...
    log.warn("Domain exception: errorCode={} message={} path={}",
  ...
  public ResponseEntity<ApiErrorResponse> handleValidation(
    ...
    return ResponseEntity.status(HttpStatus.BAD_REQUEST)
        .body(ApiErrorResponse.withFieldErrors(
            ...
            MDC.get(CorrelationIdFilter.MDC_CORRELATION_ID), fieldErrors));
  }
