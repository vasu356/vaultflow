...
public class UploadService {
  ...
  public UploadResponse uploadSinglePart(
      UUID bucketId, String objectKey, InputStream data, long contentLength,
      String contentType, String expectedChecksum, VaultFlowUserPrincipal principal) {
    ...
    if (contentLength > MAX_SINGLE_PART_SIZE) {
      throw new InvalidUploadException(
          "File exceeds single-part limit. Use multipart upload.");
    }
    ...
    String resolvedContentType = contentTypeDetector.detect(
        new ByteArrayInputStream(fileBytes), contentType, objectKey);
    ...
    if (!isDuplicate) {
      storage.store(storageKey, new ByteArrayInputStream(fileBytes),
          fileBytes.length, resolvedContentType);
    }

    StoredObject object = objectRepository
        .findByBucketIdAndObjectKeyAndIsDeletedFalse(bucketId, objectKey)
        .orElseGet(() -> objectRepository.save(StoredObject.builder()
            .bucketId(bucketId).objectKey(objectKey).contentType(resolvedContentType).build()));

    if (object.getCurrentVersionId() != null) {
      versionRepository.markAllNotLatest(object.getId());
    }

    int nextVersion = versionRepository.findMaxVersionNumber(object.getId())
        .map(n -> n + 1).orElse(1);
    String etag = '"' + checksum.substring(0, 32) + '"';

    ObjectVersion version = versionRepository.save(ObjectVersion.builder()
        .objectId(object.getId()).storageKey(storageKey).sizeBytes((long) fileBytes.length)
        .checksumSha256(checksum).etag(etag).versionNumber(nextVersion)
        .contentType(resolvedContentType).isLatest(true)
        .uploadedBy(UUID.fromString(principal.userId()))
        .refCount(isDuplicate ? (int) versionRepository.countByStorageKey(storageKey) + 1 : 1)
        .build());
    ...
    eventPublisher.publishFileUploaded(FileUploadedEvent.of(
        object.getId().toString(), version.getId().toString(),
        bucketId.toString(), principal.orgId(), principal.userId(),
        objectKey, storageKey, resolvedContentType, fileBytes.length, checksum));

    eventPublisher.publishAuditEvent(principal.orgId(), principal.userId(),
        "OBJECT_UPLOADED", "OBJECT_VERSION", version.getId().toString(),
        null, AuditEvent.AuditOutcome.SUCCESS);
    ...
    return UploadResponse.builder()
        .objectId(object.getId()).versionId(version.getId()).objectKey(objectKey)
        .storageKey(storageKey).sizeBytes(fileBytes.length).checksumSha256(checksum)
        .etag(etag).contentType(resolvedContentType).isDuplicate(isDuplicate)
        .uploadedAt(version.getCreatedAt()).build();
  }
  ...
  public InitiateUploadResponse initiateMultipartUpload(
    ...
    UploadSession session = sessionRepository.save(UploadSession.builder()
        .bucketId(request.bucketId()).objectKey(request.objectKey())
        .orgId(UUID.fromString(principal.orgId())).initiatedBy(UUID.fromString(principal.userId()))
        .contentType(request.contentType() != null ? request.contentType() : "application/octet-stream")
        .expectedSize(request.expectedSize()).totalParts(request.totalParts())
        .status(UploadStatus.INITIATED).build());

    log.info("Multipart upload initiated: sessionId={} key={}", session.getId(), request.objectKey());
    return InitiateUploadResponse.builder()
        .sessionId(session.getId()).objectKey(request.objectKey())
        .bucketId(request.bucketId()).expiresAt(session.getExpiresAt()).build();
  }

  @Transactional
  public PartUploadResponse uploadPart(
      UUID sessionId, int partNumber, InputStream data, long partSize,
      String checksum, VaultFlowUserPrincipal principal) {
    ...
    String partKey = storage.storePart(sessionId.toString(), partNumber,
        new ByteArrayInputStream(partBytes), partBytes.length);

    partRepository.findBySessionIdAndPartNumber(sessionId, partNumber)
        .ifPresentOrElse(
            existing -> {
            ...
            () -> partRepository.save(UploadPart.builder()
                .sessionId(sessionId).partNumber(partNumber).storageKey(partKey)
                .sizeBytes((long) partBytes.length).checksumMd5(md5)
                .checksumSha256(actualChecksum).build()));
    ...
    return PartUploadResponse.builder()
        .sessionId(sessionId).partNumber(partNumber).sizeBytes((long) partBytes.length)
        .checksumSha256(actualChecksum).etag('"' + md5 + '"')
        .receivedPartsCount(receivedParts).build();
  }
  ...
  public UploadResponse completeMultipartUpload(
    ...
    if (!Boolean.TRUE.equals(locked)) {
      throw new VaultFlowException("Upload completion already in progress",
          HttpStatus.CONFLICT, "COMPLETION_IN_PROGRESS");
    }
    try {
  ...
  private UploadResponse doComplete(
    ...
    if (parts.isEmpty()) throw new InvalidUploadException("No parts uploaded for session: " + sessionId);

    if (request.partNumbers() != null) {
      Set<Integer> received = parts.stream().map(UploadPart::getPartNumber).collect(Collectors.toSet());
      for (int expected : request.partNumbers()) {
        if (!received.contains(expected)) throw new InvalidUploadException("Missing part: " + expected);
      }
    }
    ...
    StoredObject object = objectRepository
        .findByBucketIdAndObjectKeyAndIsDeletedFalse(session.getBucketId(), session.getObjectKey())
        .orElseGet(() -> objectRepository.save(StoredObject.builder()
            .bucketId(session.getBucketId()).objectKey(session.getObjectKey())
            .contentType(session.getContentType()).build()));

    if (object.getCurrentVersionId() != null) versionRepository.markAllNotLatest(object.getId());

    int nextVersion = versionRepository.findMaxVersionNumber(object.getId()).map(n -> n + 1).orElse(1);
    String etag = '"' + EtagUtil.multipartEtag(parts.stream().map(UploadPart::getChecksumMd5).toList())
        + "-" + parts.size() + '"';

    ObjectVersion version = versionRepository.save(ObjectVersion.builder()
        .objectId(object.getId()).storageKey(storageKey).sizeBytes(totalSize)
        .checksumSha256(finalChecksum).etag(etag).versionNumber(nextVersion)
        .contentType(session.getContentType()).isLatest(true)
        .uploadedBy(UUID.fromString(principal.userId())).build());
    ...
    eventPublisher.publishFileUploaded(FileUploadedEvent.of(
        object.getId().toString(), version.getId().toString(),
        session.getBucketId().toString(), principal.orgId(), principal.userId(),
        session.getObjectKey(), storageKey, session.getContentType(), totalSize, finalChecksum));

    meterRegistry.counter("upload.multipart.completed").increment();
    meterRegistry.counter("upload.bytes.total").increment(totalSize);
    log.info("Multipart complete: sessionId={} parts={} size={}", sessionId, parts.size(), totalSize);

    return UploadResponse.builder()
        .objectId(object.getId()).versionId(version.getId()).objectKey(session.getObjectKey())
        .storageKey(storageKey).sizeBytes(totalSize).checksumSha256(finalChecksum)
        .etag(etag).contentType(session.getContentType()).uploadedAt(version.getCreatedAt()).build();
  }
  ...
  public void abortUpload(UUID sessionId, VaultFlowUserPrincipal principal) {
    UploadSession session = getActiveSession(sessionId, principal.orgId());
    List<UploadPart> parts = partRepository.findBySessionIdOrderByPartNumberAsc(sessionId);
    parts.forEach(p -> storage.deletePart(sessionId.toString(), p.getPartNumber(), p.getStorageKey()));
  ...
  public UploadStatusResponse getUploadStatus(UUID sessionId, VaultFlowUserPrincipal principal) {
    UploadSession session = sessionRepository.findByIdAndOrgId(sessionId,
        UUID.fromString(principal.orgId()))
        .orElseThrow(() -> new ResourceNotFoundException("UploadSession", sessionId.toString()));
    List<UploadPart> parts = partRepository.findBySessionIdOrderByPartNumberAsc(sessionId);
    return UploadStatusResponse.builder()
        .sessionId(sessionId).status(session.getStatus().name())
        .objectKey(session.getObjectKey()).bucketId(session.getBucketId())
        .totalParts(session.getTotalParts()).receivedParts(parts.size())
  ...
  public void softDeleteObject(UUID bucketId, String objectKey, VaultFlowUserPrincipal principal) {
    validateBucketAccess(bucketId, principal.orgId());
    StoredObject object = objectRepository.findByBucketIdAndObjectKeyAndIsDeletedFalse(bucketId, objectKey)
        .orElseThrow(() -> new ResourceNotFoundException("Object", objectKey));
    object.softDelete();
    objectRepository.save(object);
    eventPublisher.publishAuditEvent(principal.orgId(), principal.userId(),
        "OBJECT_DELETED", "OBJECT", object.getId().toString(), null, AuditEvent.AuditOutcome.SUCCESS);
  }

  @Transactional
  public void restoreObject(UUID bucketId, String objectKey, VaultFlowUserPrincipal principal) {
    validateBucketAccess(bucketId, principal.orgId());
    objectRepository.findByBucketIdAndObjectKeyAndIsDeletedFalse(bucketId, objectKey)
        .ifPresent(obj -> { obj.restore(); objectRepository.save(obj); });
  }
  ...
  private UploadSession getActiveSession(UUID sessionId, String orgId) {
    UploadSession session = sessionRepository.findByIdAndOrgId(sessionId, UUID.fromString(orgId))
        .orElseThrow(() -> new ResourceNotFoundException("UploadSession", sessionId.toString()));
    if (!session.isActive()) throw new InvalidUploadException("Session not active: " + session.getStatus());
